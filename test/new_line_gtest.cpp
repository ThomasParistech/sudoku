...
class NewLine_GTest : public ::testing::Test
{
protected:
    virtual void SetUp() {}
    ...
    /// @brief Tests if the class NewLine properly adding the constraint of a given value @p val
    /// @param line Line to test
    /// @param val Constraint value to set (0,1...8)
    void test_reset_and_add_constraint(NewLine line, short val)
    {
        for (int i = 0; i < 9; i++)
        {
    ...
    void apply_constraints_on_line(NewLine &line, short val, const std::vector<int> &position_constraints,
                                   std::vector<NewLine::Status> &output_status, std::vector<int> &output_values,
...
TEST_F(NewLine_GTest, reset_and_add_constraint)
{
...
TEST_F(NewLine_GTest, repeat_same_constraint)
{
...
TEST_F(NewLine_GTest, setvalue_left_to_rigtht)
{
    const int val = 4;
    std::vector<int> position_constraints;
    std::vector<NewLine::Status> output_status;
    ...
    for (int pos_ref = 0; pos_ref < 9; pos_ref++)
    {
        ...
        ASSERT_EQ(NewLine::Status::SET_VALUE, output_status.back()) << " At pos_ref= " << pos_ref;
        for (int i = 0; i < 7; i++)
            ASSERT_NE(NewLine::Status::SET_VALUE, output_status[i]) << " At pos_ref= " << pos_ref << " and i=" << i;
...
TEST_F(NewLine_GTest, setvalue_right_to_left)
{
    const int val = 4;
    std::vector<int> position_constraints;
    std::vector<NewLine::Status> output_status;
...
TEST_F(NewLine_GTest, lock)
{
    ...
    std::vector<NewLine::Status> gt_final_statuses{
        NewLine::Status::LOCK,
        NewLine::Status::LOCK,
        NewLine::Status::LOCK,
        NewLine::Status::SET_VALUE,
        NewLine::Status::SET_VALUE,
        NewLine::Status::SET_VALUE};
    ...
    for (int k = 0; k < position_constraints.size(); k++)
    {
        ...
        for (int i = 0; i < output_status.size() - 1; i++)
            ASSERT_EQ(NewLine::Status::NOTHING, output_status[i]) << " At k= " << k;
