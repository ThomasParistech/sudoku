...
void Matrix::decreaseNbr(int val, Cell* cell, std::vector<CellToAdd>& cellsToAdd, std::vector<CellToLock>& cellsToLock, std::vector<Cell*>& cellsToPair){
	...
	if(cell->isFree()){
         switch(resultCell) {
            case Cell::TO_DO::SETVALUE : cellsToAdd.push_back(CellToAdd(i, j, (*cell).findNew()));
                            break;
            case Cell::TO_DO::PAIR : cellsToPair.push_back(cell);
                         break;
            case Cell::TO_DO::NOTHING : break;
        }
	}
	...
	if(rows[i].getTotalNbrPossible(val)!= -3){
        switch(rows[i].decreaseNbr(val, jBlock)) {
            case Line::TO_DO::SETVALUE : cellsToAdd.push_back(findNewInRow(val, i));
                            break;
            case Line::TO_DO::LOCK_SQUARE0 : cellsToLock.push_back(std::make_pair(val, getSquareLockedRow(iBlock, 0, i)));
                         break;
            case Line::TO_DO::LOCK_SQUARE1 : cellsToLock.push_back(std::make_pair(val, getSquareLockedRow(iBlock, 1, i)));
                         break;
            case Line::TO_DO::LOCK_SQUARE2 : cellsToLock.push_back(std::make_pair(val, getSquareLockedRow(iBlock, 2, i)));
                         break;
            case Line::TO_DO::NOTHING : break;
        }
	...
	if(cols[j].getTotalNbrPossible(val)!= -3){
	   	switch(cols[j].decreaseNbr(val, iBlock)) {
            case Line::TO_DO::SETVALUE : cellsToAdd.push_back(findNewInCol(val, j));
                            break;
            case Line::TO_DO::LOCK_SQUARE0 : cellsToLock.push_back(std::make_pair(val, getSquareLockedCol(0, jBlock, j)));
                         break;
            case Line::TO_DO::LOCK_SQUARE1 : cellsToLock.push_back(std::make_pair(val, getSquareLockedCol(1, jBlock, j)));
                         break;
            case Line::TO_DO::LOCK_SQUARE2 : cellsToLock.push_back(std::make_pair(val, getSquareLockedCol(2, jBlock, j)));
                         break;
            case Line::TO_DO::NOTHING : break;
        }
	...
	if(blocks[iBlock][jBlock].getTotalNbrPossible(val)!= -3){
        switch(blocks[iBlock][jBlock].decreaseNbr(val, iBlock, jBlock, i, j)) {
            case Square::TO_DO::SETVALUE : cellsToAdd.push_back(findNewInSquare(val, iBlock, jBlock));
                            break;
            case Square::TO_DO::LOCK_ROW0 : cellsToLock.push_back(std::make_pair(val, getRowLockedSquare(iBlock, jBlock, 0)));
                         break;
            case Square::TO_DO::LOCK_ROW1 : cellsToLock.push_back(std::make_pair(val, getRowLockedSquare(iBlock, jBlock, 1)));
                         break;
            case Square::TO_DO::LOCK_ROW2 : cellsToLock.push_back(std::make_pair(val, getRowLockedSquare(iBlock, jBlock, 2)));
                         break;

            case Square::TO_DO::LOCK_COL0 : cellsToLock.push_back(std::make_pair(val, getColLockedSquare(iBlock, jBlock, 0)));
                         break;
            case Square::TO_DO::LOCK_COL1 : cellsToLock.push_back(std::make_pair(val, getColLockedSquare(iBlock, jBlock, 1)));
                         break;
            case Square::TO_DO::LOCK_COL2 : cellsToLock.push_back(std::make_pair(val, getColLockedSquare(iBlock, jBlock, 2)));
                         break;
            case Square::TO_DO::NOTHING : break;
        }
	}
}
...
void Matrix::update(int valRestrict, std::vector<Cell*> cellsUpdated){      // the value has just been updated. We update the restrictions on the neighbors
	...
	for(auto c : cellsToPair){
        if(c->nbrPossible == 2){// it might have change with all the other updates
            int k1 = -1, k2 = -1;
            for(int k=0; k<9;k++){
                if(c->possible[k]){
                    if(k1 == -1)
    ...
    for(int i=0; i<9; i++){
        for(int j=0; j<9; j++){
            if(coloringPaths[i][j]==0 && cells[i][j].possible[val]){
                bool realPath = false; int lengthPath = 0;
